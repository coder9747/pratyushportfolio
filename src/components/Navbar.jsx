    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between  items-center max-w-7xl mx-auto'>
        ...
        >
          ...
          <p className='text-white text-[18px]  font-bold cursor-pointer'>Pratyush's Portfolio</p>
        </Link>
            ...
            <ul className='list-none  flex flex-col items-start justify-end gap-10'>
              {navLinks.map((item) => {
                ...
                return (<li
                  ...
                  className={`${active === item.title ? "text-white" : "text-secondary"} hover:text-white text-[16px] font-poppins font-medium cursor-pointer `}
                >
                  <a href={`#${item.id}`}>{item.title}</a>
                </li>)
              })}
